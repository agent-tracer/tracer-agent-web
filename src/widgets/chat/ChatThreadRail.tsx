...
import { chatThreadDisplayTitle, DEFAULT_CHAT_THREAD_TITLE, type ChatThreadRecord } from "~/entities/chat/model/chat.js";
import { useCreateThreadMutation, useDeleteThreadMutation } from "~/entities/chat/api/mutations.js";
import { useGuidance } from "tracerWeb/store";
import { Button, GuidanceText, IconButton, Modal, ScrollArea, TrashIcon } from "tracerWeb/ui";
...
function ThreadRow({ thread, selected, onSelect, onDelete }: ThreadRowProps) {
  ...
  return (
    <div
      className={cn(
        "group grid grid-cols-[minmax(0,1fr)_28px] items-center rounded-xs transition-colors",
    ...
    >
      ...
      <IconButton
        ...
        className={cn(
          "justify-self-center border-transparent text-ink-tertiary hover:text-err",
        )}
      >
        <TrashIcon />
      </IconButton>
