...
import { formatAbsoluteHHmmss, formatDuration, formatRelativeShort } from "~/widgets/jobs/lib/time.js";
import { useGuidance } from "tracerWeb/store";
import { Button, Modal, Pill } from "tracerWeb/ui";
import { JOB_KIND_LABEL, JOB_STATUS_LABEL, elapsedMs, summarizeResult } from "~/widgets/jobs/lib/job-view.js";
import { job } from "tracerWeb/entities";

const { JOB_STATUS, isCancelableJobStatus, useCancelJobMutation } = job;
      ...
      >
        <div className="flex justify-end gap-2">
          ...
          <Button
            variant="primary"
            onClick={() => {
              cancel.mutate(job, { onSettled: () => setConfirming(false) });
            }}
            disabled={cancel.isPending}
          >
            Cancel job
